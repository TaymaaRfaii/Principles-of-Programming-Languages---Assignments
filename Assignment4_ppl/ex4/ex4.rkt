...
(define pipe$
  (lambda (fs contfunc1)  
    (if (empty? (cdr fs))
        (contfunc1 (lambda (x contfunc2) ((car fs) x contfunc2)))
        (pipe$ (cdr fs)
               (lambda (result) 
                       (compose$ (car fs) result contfunc1))))))
; compose function as CPS       
(define compose$
  (lambda (f$ g$ contfunc1)
     (contfunc1 (lambda (x contfunc2)
            (f$ x (lambda (result) (g$ result contfunc2)))))))
...
(define generate-pi-approximations
  (lambda ()
    (reduce3-lzl + 0 (lzl-map hmode (integers-steps-from 1 4)))
   )
 )
(define hmode
  (lambda (a)
