...
import {isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp, makeAnyTExp,makePredicateTExp} from './TExp';
...
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
    if (isArray(vars)) {
        ...
        const body = mapResult(parseL5CExp, rest[0] === ":" && rest[1]=== "is?" ? rest.slice(3) :
            rest[0] === ":" && rest[1]!== "is?"? rest.slice(2):
                rest
        );
        //
        const returnTE = rest[0] === ":"&&rest[1]=== "is?"?(bind(parseTExp(rest[2]),(texp:TExp)=> makeOk(makePredicateTExp(makeAnyTExp(),texp)))):
        rest[0] === ":"&&rest[1]!== "is?"? parseTExp(rest[1]) : makeOk(makeFreshTVar());
