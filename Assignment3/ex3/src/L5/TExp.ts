...
export type CompoundTExp = ProcTExp | TupleTExp | UnionTExp|InterTExp|PredicateTExp ;
export const isCompoundTExp = (x: any): x is CompoundTExp => isProcTExp(x) || isTupleTExp(x) || isUnionTExp(x)||isInterTExp(x)||isPredicateTExp(x);
...
//Last
export type PredicateTExp = { tag: "PredicateTExp"; paramTEs: TExp; returnTE: TExp;};
export const makePredicateTExp = (paramTEs: TExp, returnTE: TExp): PredicateTExp =>
    ({tag: "PredicateTExp", paramTEs: paramTEs, returnTE: returnTE});
...
const flattenS = (tes: TExp[]): TExp[] =>
    norm(sort(superTypeComparator, flattenInter(tes)));
...
const normalizeInter = (ute: InterTExp): TExp =>
    dnf(isEmpty(ute.components) ? makeNeverTExp() :
...
const norm = (tes: TExp[]): TExp[] =>
    isEmpty(tes) ? tes :
        !containsType(tes.slice(1), tes[0]) ? norm(tes.slice(1)) :
            isAnyTExp(tes[0]) ? norm(tes.slice(1)) :
                [tes[0], ...norm(tes.slice(1))];
// L52 END
export const flagger = (tes: TExp[]): TExp =>
    normalizeInter(({tag: "InterTExp", components: flattenS(tes)}));
...
export const isSubType = (te1: TExp, te2: TExp): boolean =>
    ...
    isUnionTExp(te2) ? containsType(te2.components, te1) :
        //Hmode-Check
        <boolean>isNeverTExp(te1) ? true:
            <boolean>isAnyTExp(te2) ? true:
    ...
    (isProcTExp(te1) && isProcTExp(te2)) ? checkProcTExps(te1, te2) :
        //Last
        isPredicateTExp(te1) ? isSubType(makeBoolTExp(), te2) :
            isPredicateTExp(te2) ? isSubType(te1, makeBoolTExp()) :
...
//           isUnionTExp(te1) ? (!isInterTExp(te2)?(isSubType(te1.components[0],te2)?
//               makeDiffTExp(makeUnionTExp(te1.components.slice(1)),te2):
//               [te1.components[0], makeDiffTExp(makeUnionTExp(te1.components.slice(1)),te2)]):(isSubType(te1.components[0],asdte2)?
//                   makeDiffTExp(makeUnionTExp(te1.components.slice(1)),te2):
//                   [te1.components[0], makeDiffTExp(makeUnionTExp(te1.components.slice(1)),te2)])):
//               isInterTExp(te1)? (te1):
//                   isAtomicTExp(te1) && equals(te1,te2)? makeNeverTExp():
    ...
    isNeverTExp(te2) ? te1:
        //isAnyTExp(te2) ? makeNeverTExp():
          ...
          isAnyTExp(te2) ? makeNeverTExp():
          isAnyTExp(te1) ? makeAnyTExp():
              isAtomicTExp(te1)&& !equals(te1,te2) ? te1:
                  isAtomicTExp(te1)? makeNeverTExp():
          isUnionTExp(te1)&& isSubType(te1.components[0],te2) &&!isInterTExp(te2) ? makeDiffTExp(makeUnionTExp(te1.components.slice(1)),te2):
              isUnionTExp(te1)&& !isSubType(te1.components[0],te2)&&!isInterTExp(te2) ? makeUnionTExp([te1.components[0], makeDiffTExp(makeUnionTExp(te1.components.slice(1)),te2)]):
                  isUnionTExp(te1)&& isInterTExp(te2)&&isSubType(te1.components[0],flagger(te2.components))  ? makeDiffTExp(makeUnionTExp(te1.components.slice(1)),te2):
                      isUnionTExp(te1)&&isInterTExp(te2)&& !isSubType(te1.components[0],flagger(te2.components)) ? makeUnionTExp([te1.components[0], makeDiffTExp(makeUnionTExp(te1.components.slice(1)),te2)]):
              isInterTExp(te1)&& isSubType(te1.components[0],te2)&&!isInterTExp(te2) ? makeDiffTExp(makeInterTExp(te1.components.slice(1)),te2):
                  isInterTExp(te1)&& !isSubType(te1.components[0],te2)&&!isInterTExp(te2) ? makeInterTExp([te1.components[0], makeDiffTExp(makeInterTExp(te1.components.slice(1)),te2)]):
                      isInterTExp(te1)&&isInterTExp(te2)&& isSubType(te1.components[0],flagger(te2.components)) ? makeDiffTExp(makeInterTExp(te1.components.slice(1)),te2):
                          isInterTExp(te1)&&isInterTExp(te2)&& !isSubType(te1.components[0],flagger(te2.components)) ? makeInterTExp([te1.components[0], makeDiffTExp(makeInterTExp(te1.components.slice(1)),te2)]):
                        te1
...
*/
const parseProcTExp = (texps: Sexp[]): Result<ProcTExp|PredicateTExp> => {
    //Last
    if(texps.indexOf('is?')> -1&&texps.indexOf('->')>-1){
        const pos = texps.indexOf('->');
        return (pos === -1)  ? makeFailure(`Procedure type expression without12 -> - ${format(texps)}`) :
            (pos === 0) ? makeFailure(`No param types in proc texp - ${format(texps)}`) :
                (pos === texps.length - 1) ? makeFailure(`No return type in proc texp - ${format(texps)}`) :
                    (pos !== 1) ? makeFailure('Multiple param types in proc texp - ${format(texps)}'):
                    (texps.slice(pos + 1).indexOf('->') > -1) ? makeFailure(`Only one -> allowed in a procexp - ${format(texps)}`) :
                        bind(parseTExp(texps[0]), (args: TExp) =>
                            bind(parseTExp(texps[pos + 2]), (returnTE: TExp) =>
                                makeOk(makePredicateTExp(args, returnTE))));
    }
    const pos = texps.indexOf('->');
    return (pos === -1)  ? makeFailure(`Procedure type expression without34 -> - ${format(texps)}`) :
        (pos === 0) ? makeFailure(`No param types in proc texp - ${format(texps)}`) :
            (pos === texps.length - 1) ? makeFailure(`No return type in proc texp - ${format(texps)}`) :
                (texps.slice(pos + 1).indexOf('->') > -1) ? makeFailure(`Only one -> allowed in a procexp - ${format(texps)}`) :
            ...
            isInterTExp(x) ? mapv(mapResult(unparseTExp, x.components), (componentTEs: string[]) =>
                ...
                isProcTExp(x) ? bind(unparseTuple(x.paramTEs), (paramTEs: string[]) =>
                        mapv(unparseTExp(x.returnTE), (returnTE: string) =>
                            [...paramTEs, '->', returnTE])) :
                    //Last
                    isPredicateTExp(x)? bind(unparseTExp(x.paramTEs), (paramTEs: string) =>
                            mapv(unparseTExp(x.returnTE), (returnTE: string) =>
                                [...paramTEs, '->', returnTE])) :
