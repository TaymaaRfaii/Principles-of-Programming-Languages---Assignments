...
import {
    ...
    BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isSubType, isPredicateTExp, PredicateTExp
} from "./TExp";
...
//Last
const isTypePredApp = (e: Exp, tenv: TEnv): Result<PredicateTExp> =>
    bind(typeofExp(e,tenv),(texp:TExp) => isPredicateTExp(texp) ? makeOk(texp):makeFailure("Not a pred!"))


export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> =>
    either(
        bind (isTypePredApp(ifExp.test, tenv), (pred:PredicateTExp) => makeOk(pred.returnTE)),
        makeOk,
