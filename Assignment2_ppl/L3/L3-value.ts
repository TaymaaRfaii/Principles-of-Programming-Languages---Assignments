...
export const makeClassEnv = (fields: VarDecl[], methods: Binding[], env: Env): Class =>
    ({tag: "ClassExp", fields: fields, methods: methods, env: env });

export const isClassExp = (x: any): x is Class => x.tag === "ClassExp";
//Added: Object___ ____ ____ ____ _____ _____ ___ ____ ____ ____
export type Object = { 
    tag: "Object";
    methods: Binding[];
    env: Env;
...
//Added Object & Class to SExpValue ..
export type SExpValue = number | boolean | string | PrimOp | Closure | SymbolSExp | EmptySExp | CompoundSExp |Object| Class;
...
export const compoundSExpToString = (cs: CompoundSExp, css = compoundSExpToArray(cs, [])): string => 
    ...
    `(${css.s1.join(' ')} . ${css.s2})`
//Added_______________________________________________
export const classSExpToString = (cv: Class): string => 
    `Class`
...
export const valueToString = (val: Value): string =>
    ...
    isClassExp(val) ? classSExpToString(val) : //added
    val ; 
