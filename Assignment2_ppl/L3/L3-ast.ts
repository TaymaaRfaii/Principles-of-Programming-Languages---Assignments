...
//Added: type predicate for classExp
...
//Added isClassExp(x) to the function isCompoundExp    
...
export const parseL3SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    makeFailure("Never");
//added
export const parseClassExp = (varDecl: Sexp,bindings: Sexp): Result<ClassExp> => {
    if (!(isArray(varDecl) && allT(isString, varDecl))) {
        ...
        const fields : VarDecl[] = map(makeVarDecl, varDecl);
